...
2026-09-17 review 修訂；[來源](sources.md)、[完整 gate registry](review-gates.md)。下列上游 wire 來自固定 v1.5.0；本頁記錄規劃 PR 的契約，policy 仍是 proposed schema，尚無完整 compiler/runtime。來源查核、helper 自測與真實 Gateway E2E 是不同證據。
...
G0-WIRE 區分「原版漏洞診斷」與「受保護部署」。原版注入負向 fixture 應重現問題，狀態 VULNERABILITY_REPRODUCED，不是 protected PASS。部署必須修補 parser 或驗收不可繞過的 webhook response validation boundary；本規劃變更沒有該修補。
...
矩陣同時綁 gateway_image/gateway_config/compiler/adapter 的 SHA-256，artifact 本身的 hash 必須來自可信發布目錄。記錄 source JSON pointer、轉換位置、lossiness、hook/上游觀察、native enforcement、fixture/artifact hash；狀態 observed_inspected/ingress_rejected/forwarded_uninspected/unknown。route/protocol/phase/schema scope 必須一致；missing、expired、future-dated、digest mismatch 或不可信 provenance 均拒絕。只可對宣告 schema/內容義務聲稱覆蓋，不推論未知未來欄位或任意語意完整性。

未知原始欄位必須 normalize 前拒絕；hook 無法知道已丟棄的資料。缺原始 envelope 檢查或有效 field matrix，不可把 incompleteCoverage: deny 標為滿足。未支援 image_url 等可用已證明 ingress reject 滿足封閉 profile，不要求硬塞進文字 detector。證據輸出位置見 review-gates；規劃 PR 不提供人工填 PASS 的 runtime coverage matrix。

| 表面 | MVP 政策 | 升級條件 |
|---|---|---|
| /v1/chat/completions 文字、非串流 | G0 四項通過後才支持 | history/choices/body limits/mutation、context、原始欄位驗收 |
| LLM tool/function-call fields | unverified 即拒絕該功能 | 逐欄位 ingress/hook/egress fixture 與安全 mutation |
...
工具列表與執行授權共用 catalog，但每次 tools/call 重新檢查。Cache key 含 tenant/auth scope/backend catalog/policy revision；pagination/list_changed/fanout/callback 行為用真實 fixture，不猜測 multiplexed 名稱。Session 不可保留已撤銷權限。
...
[獨立 YAML](../examples/policies/strict-local.proposed.yaml) 是本提案的唯一 canonical policy 範例，本文件不再複製第二份 YAML，避免 schema 漂移。它不是原生 Gateway configuration；P1 仍需建立 schema/compiler/migration，禁止未知欄位和任意 script。

`deploymentRequirements.fieldCoverageOracle` 是物件，`mode=external_digest_bound`；`bindTo` 包含 gateway_image、gateway_config、compiler、adapter；必需 artifact hash、trusted provenance，且 rejectMissing/rejectExpired/rejectFutureDated/rejectDigestMismatch 全為 true。`rejectUnknownFieldsBeforeNormalization` 仍為 true。

`compatibilityEvidenceRef: null` 故意表示缺證據，compiler 必須拒絕 activation；不能把本範例當已部署 policy。Future-dated evidence rejection 是 strict compiler 不可停用的安全 invariant，不能以 policy override 解除。
...
pplx 原版預設截斷至 4,096 tokens，需 tokenizer-aware chunks/overlap/原文 span mapping；不能把未讀資料標已掃描。[S6] chunking 不代表跨 chunk/跨輪 injection 語意完整。Base64/zero-width 等僅建立 bounded analysis view，不執行內容、不自動改寫原文；decode depth/expansion bytes/chunk count/deadline 固定並測試，roleplay 不當編碼問題處理。
